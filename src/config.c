...
{
	if (eps >= 0.5) {
		syslog(LOG_ERR, "Timer epsilon cannot be equal-to or greater than 0.5!");
		return 1;
	} else if (eps > 0.10) {
		syslog(LOG_WARNING, "Timer epsilon very high!");
	} else if (eps <= 0) {
		syslog(LOG_ERR, "Timer epsilon cannot less than or equal to 0!");
		return 1;
	}
	return 0;
}
...
int sane_flush_interval(int intv)
{
	if (intv <= 0) {
		...
		syslog(LOG_WARNING, "Flushing set to be very infrequent! Increased risk of data loss.");
	}
	return 0;
}
...
int sane_histograms(histogram_config * config)
{
	while (config) {
		...
		if (config->min_val >= config->max_val) {
			syslog(LOG_ERR, "Histogram min value must be less than max value! Prefix: %s",
				config->prefix);
			return 1;
		}
		// Check width
		if (config->bin_width <= 0) {
			syslog(LOG_ERR, "Histogram bin width must be greater than 0! Prefix: %s",
				config->prefix);
			return 1;
		}
